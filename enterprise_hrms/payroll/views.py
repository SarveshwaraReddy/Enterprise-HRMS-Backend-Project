...
class PayslipViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        if not file_path or not os.path.exists(file_path):
            PayrollService.generate_payslip(payslip.employee, payslip.payroll_run)
...
class PayrollReportsView(views.APIView):
    ...
    def get(self, request, report_type=None):
        ...
        if report_type == 'summary':
            ...
            fmt = request.query_params.get('export_format', request.query_params.get('format', 'csv')).lower()
            if not month or not year:
